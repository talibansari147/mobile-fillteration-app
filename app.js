...
brand.addEventListener("change", () => {
    mobileModel.innerHTML = `<option value="">Select Model</option>`;
    mobileModel.disabled = true;
...
mobileModel.addEventListener("change", () => {
    searchBtn.disabled = mobileModel.value === "";
});

searchBtn.addEventListener("click", () => {
    let selectedBrand = brand.value;
    let selectedModel = mobileModel.value;

    if (selectedBrand && selectedModel) {
        let d = mobiles[selectedBrand][selectedModel];

        let output = `
            <h2>${selectedBrand.toUpperCase()} - ${selectedModel}</h2>
        `;

        for (let key in d) {
            if (typeof d[key] === "object") {
                output += `<strong>${key.toUpperCase()}:</strong><br>`;
                for (let innerKey in d[key]) {
                    if (typeof d[key][innerKey] === "object") {
                        output += `&nbsp;&nbsp;<b>${innerKey}:</b><br>`;
                        for (let sub in d[key][innerKey]) {
                            output += `&nbsp;&nbsp;&nbsp;&nbsp;- ${sub}: ${d[key][innerKey][sub]}<br>`;
                        }
                    } else {
                        output += `&nbsp;&nbsp;- ${innerKey}: ${d[key][innerKey]}<br>`;
                    }
                }
                output += `<br>`;
            } else {
                output += `<strong>${key}:</strong> ${d[key]}<br>`;
            }
        }
